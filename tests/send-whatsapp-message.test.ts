...
import type { WhatsAppCloudApiClient } from '../src/tools/whatsapp-cloud-api.client.js';

function makeClient(override: Partial<WhatsAppCloudApiClient>): WhatsAppCloudApiClient {
  return override as WhatsAppCloudApiClient;
}
  ...
  it('returns failure with error details on API error', async () => {
    const client = makeClient({
      sendMessage: vi.fn().mockRejectedValue(
        new WhatsAppApiClientError('Invalid token', 190, 'OAuthException'),
      ),
    });
